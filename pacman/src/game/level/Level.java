...
public class Level {
	...
	public static int numberOfFoods = 3; // total number of foods left in game, intially 3 foods: poop, banana,
											// strawberry
	...
	private void computeScore() {
		...
		if (diff != 0) {
			...
			if (!Game.getSound().isPlaying(1))
				Game.getSound().play(1); // chomp
		}
	}
	...
	private void playerDead() {
		...
		if (!canEat) {
			...
			for (int i = 0; i < enemies.size(); i++) {
				TileCoordinate ghostTileCoord = new TileCoordinate(((int) enemies.get(i).getX() >> 4),
						((int) enemies.get(i).getY() >> 4));
				if (sameLocation(playerTileCoord, ghostTileCoord)) {
					...
					for (int k = 0; k < enemies.size(); k++) {
						...
						if (e instanceof OrangeGhost)
							((OrangeGhost) e).setLocation(16, 15);
						if (e instanceof RedGhost)
							((RedGhost) e).setLocation(17, 14);
						if (e instanceof PinkGhost)
							((PinkGhost) e).setLocation(15, 16);
						if (e instanceof GreenGhost)
							((GreenGhost) e).setLocation(17, 16);
						if (e instanceof BlueGhost)
	...
	private void turnGhostsScared() {
		...
		for (int i = 0; i < flashDotsRemaining.size(); i++) {
			TileCoordinate flashDotCoord = new TileCoordinate(((int) flashDotsRemaining.get(i).getX() >> 4),
					((int) flashDotsRemaining.get(i).getY() >> 4));
	...
	private void foodsEaten() {
		...
		for (int i = 0; i < foods.size(); i++) {
			TileCoordinate foodTileCoord = new TileCoordinate(((int) foods.get(i).getX() >> 4),
					((int) foods.get(i).getY() >> 4));
	...
	public void update() {
		...
		if (canEat)
			ghostEaten(); // checks if player hits same spot as that ghost and removes the ghost

		if (countScaredTime)
			totalScaredTime++; // timer begins
		...
		if (spawnFoods && time < 3601 && time % 3600 == 0) { // spawns 60 seconds into the game
			if (!foods.isEmpty())
				foods.remove(0); // removes Poop ONLY if player has not eaten it
			add(new Banana(2, 2));
		}
		if (spawnFoods && time < 6001 && time % 6000 == 0) { // spawns after 100 seconds from game start
			if (!foods.isEmpty())
				foods.remove(0); // removes Banana ONLY if player has not eaten it
		...
		if (dots.size() > 0 && dots.size() <= SPEED_BOOST_THRESHOLD)
			setGhostSpeed(2.0);

		if (dots.isEmpty())
			Game.state = Game.state.GAMEWIN; // if no dots left, player has won, change state
		if (reset)
			resetGameLevel(); // check to replay
	}
	...
	public Tile getTile(int x, int y) {
		...
		if (tiles[x + y * width] == Tile.col_spawn_background)
			return Tile.spawnBackgroundTile;
		if (tiles[x + y * width] == Tile.col_spawn_RightEdge)
			return Tile.spawnRightEdgeTile;
		if (tiles[x + y * width] == Tile.col_spawn_LeftEdge)
			return Tile.spawnLeftEdgeTile;
		if (tiles[x + y * width] == Tile.col_spawn_TopEdge)
			return Tile.spawnTopEdgeTile;
		if (tiles[x + y * width] == Tile.col_spawn_BottomEdge)
			return Tile.spawnBottomEdgeTile;
		if (tiles[x + y * width] == Tile.col_spawn_TLCorner)
			return Tile.spawnTLCornerTile;
		if (tiles[x + y * width] == Tile.col_spawn_TRCorner)
			return Tile.spawnTRCornerTile;
		if (tiles[x + y * width] == Tile.col_spawn_BLCorner)
			return Tile.spawnBLCornerTile;
		if (tiles[x + y * width] == Tile.col_spawn_BRCorner)
			return Tile.spawnBRCornerTile;
		if (tiles[x + y * width] == Tile.col_spawn_LeftSide)
			return Tile.spawnLeftSideTile;
		if (tiles[x + y * width] == Tile.col_spawn_RightSide)
			return Tile.spawnRightSideTile;
		if (tiles[x + y * width] == Tile.col_spawn_TopSide)
			return Tile.spawnTopSideTile;
		if (tiles[x + y * width] == Tile.col_spawn_BottomSide)
			return Tile.spawnBottomSideTile;
